  ...
  <CustomerForm onCustomerCreated={(newCustomer) => setCustomers([...customers, newCustomer])} />
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
        {customers.map((customer) => (
          <div
            key={customer._id}
            className={`border p-4 rounded-lg cursor-pointer bg-[#1f2d47] shadow hover:bg-[#282354] transition ${
              selectedCustomer?._id === customer._id ? 'border-blue-400' : 'border-gray-600'
            }`}
            onClick={() => setSelectedCustomer(customer)}
          >
            <h4 className="font-bold text-lg">{customer.name}</h4>
